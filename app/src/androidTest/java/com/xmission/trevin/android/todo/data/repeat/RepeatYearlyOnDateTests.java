...
public class RepeatYearlyOnDateTests {
    ...
    /**
     * Specific case for a random run of
     * {@link #testRepeatEveryNYearsRestrictedByDayWithEndPrevious()}
     * which resulted in an edge case failure:
     * <ul>
     *     <li>Interval is every 3 years on or the prior
     *     Sunday, Tuesday, Wednesday, or Thursday.</li>
     *     <li>End date was 2069-01-01, but that&rsquo;s
     *     not relevant to the failure.</li>
     *     <li>Initial due date was Thursday, 2004-01-01.</li>
     *     <li>Expected due date is Sunday, 2006-12-31.</li>
     *     <li>Actual due date before the bug was fixed was
     *     Tuesday, 2008-01-01</li>
     * </ul>
     */
    @Test
    public void testRepeatEvery3YearsJan1SunTueWedThur() {
        ...
        repeat.setAllowedWeekDays(Set.of(WeekDays.SUNDAY,
                WeekDays.TUESDAY, WeekDays.WEDNESDAY, WeekDays.THURSDAY));
        repeat.setDirection(WeekdayDirection.PREVIOUS);
        repeat.setEnd(LocalDate.of(2069, 1, 1));
        LocalDate completed = LocalDate.of(2004, 10, 10);
        LocalDate expectedDue = LocalDate.of(2006, 12, 31);
        LocalDate actualDue = repeat.computeNextDueDate(targetDate, completed);
        assertEquals("Next due date for task due Tuesday, 1 January 2004,"
                + " every 3 years on the prior Sunday, Tuesday,"
                + " Wednesday, or Thursday", expectedDue, actualDue);
    }

    /**
     * Specific edge case detected by Gemini: if the target date is
     * February 29 but the adjusted date falls into March,
     * the date cap check for February may fail.
     */
    @Test
    public void testRepeatEveryYearFeb29NextSunMonTue() {
        LocalDate targetDate = LocalDate.of(2008, 2, 29);
        RepeatYearlyOnDate repeat = new RepeatYearlyOnDate(targetDate);
        repeat.setDate(targetDate.getDayOfMonth());
        repeat.setMonth(Months.fromJavaMonth(targetDate.getMonth()));
        repeat.setAllowedWeekDays(Set.of(WeekDays.SUNDAY,
        ...
        LocalDate completed = LocalDate.of(2008, 3, 2);
        LocalDate expectedDue = LocalDate.of(2009, 3, 1);
        LocalDate actualDue = repeat.computeNextDueDate(completed, completed);
        assertEquals("Next due date for task due Sunday, 2 March 2008,"
                + " every year on the next Monday, Tuesday, or Friday of February 29",
                expectedDue, actualDue);
    }
