...
public class RepeatSettings implements Cloneable {
	...
	private static SparseArray<IntervalType> idMap =
	    new SparseArray<>();
	public final int value;
	IntervalType(int v) { value = v; }
	static {
    ...
    static {
	TreeSet<Integer> weekDays = new TreeSet<>();
	...
	public final int value;
	WeekdayDirection(int v) { value = v; }
    ...
    List<OnRepeatChangeListener> listeners = new LinkedList<>();
	...
	case YEARLY_ON_DATE:
	case YEAR_AFTER:
	    allowedWeekDays = new TreeSet<>(ALL_WEEK_DAYS);
	    break;
	...
	if (fixedWeekDays != null) {
		...
		if (fixedWeekDays.size() == 1) {
			...
			oldDays = new TreeSet<>(fixedWeekDays);
		    fixedWeekDays.clear();
		}
	...
	if (flag) {
		...
		if (fixedWeekDays.size() == 0) {
			...
			Set<Integer> oldDay = new TreeSet<>();
			oldDay.add(day);
			for (OnRepeatChangeListener listener : listeners)
			    listener.onFixedWeekdaysChanged(this,
				    Collections.unmodifiableSet(fixedWeekDays),
	...
	try {
	    ...
	    if (allowedWeekDays != null)
		copy.allowedWeekDays = new TreeSet<>(allowedWeekDays);
	    ...
	    if (fixedWeekDays != null)
		copy.fixedWeekDays = new TreeSet<>(fixedWeekDays);
